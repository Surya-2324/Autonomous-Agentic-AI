...
def get_api_key(key_name: str) -> str | None:
    """Load a secret from Streamlit Secrets first, then fall back to env vars."""
    try:
...
if not GROQ_API_KEY or not TAVILY_API_KEY:
    st.error("API keys missing! Please check Streamlit Secrets or your .env file.")
    st.stop()
...
def macro_intelligence_agent(state: AgentState) -> dict:
    topic = state.get("search_topic") or "current macroeconomic news impact on global markets"
    logger.info("Fetching live data for topic: %s", topic)
...
def market_strategy_agent(state: AgentState) -> dict:
    ...
    return {
        "proposed_strategy": response.content,
        "decision_log": ["Strategy Agent analyzed macro trends and proposed a plan."],
    }


def confidence_agent(state: AgentState) -> dict:
...
def auditor_agent(state: AgentState) -> dict:
    ...
    return {
        "audit_result": audit_decision.content,
        "decision_log": ["Auditor Agent completed formal policy compliance check."],
    }
